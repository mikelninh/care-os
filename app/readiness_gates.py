...
GATES: tuple[Gate, ...] = (
    ...
    Gate(
        ...
        claim="Source-native FHIR passes through the mandatory truth contract; untrusted document/model candidates require exact source evidence before promotion. The legacy document benchmark remains below the clinical release bar.",
        evidence=("app/clinical_truth.py", "app/fhir_adapter.py", "app/document_pipeline.py", "app/extractors/base.py", "app/extractors/model_schema.py", "app/contradictions.py", "tests/test_clinical_truth.py", "tests/test_fhir_truth.py", "tests/test_document_pipeline.py", "tests/test_verified_extraction.py", "tests/test_model_extraction_schema.py", "tests/test_contradictions.py", "docs/BENCHMARK.md"),
    ...
    Gate(
        ...
        status=GateStatus.PARTIAL,
        claim="Current/stale/unavailable source semantics, bounded pagination, fail-visible connector errors, runtime kill switches and rollback/incident rules exist; system-wide resilience/recovery evidence is still missing.",
        evidence=("app/source_state.py", "tests/test_source_state.py", "app/fhir_adapter.py", "tests/test_fhir_paging.py", "app/connectors/fhir_connector.py", "app/kill_switch.py", "tests/test_clinical_session.py", "docs/DEPLOYMENT_RUNBOOK.md", "docs/INCIDENT_RESPONSE.md", "docs/SLO_POLICY.md", "docs/THREAT_MODEL.md"),
        blockers=("failure-injection suite across KIS/FHIR/identity/audit/model dependencies", "freshness policies wired to every real source", "backup/restore evidence with measured RPO/RTO", "operational monitoring/alerting against approved SLOs", "incident/rollback exercise in target deployment"),
