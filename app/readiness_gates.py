...
GATES: tuple[Gate, ...] = (
    ...
    Gate(
        ...
        claim="FHIR transport, fail-closed paging, ISiK-oriented validation, a reusable connector contract, hospital capability manifest, adapter-maturity catalog and synthetic/deidentified FHIR-family data-plane scaffold exist. Real KIS/LIS/vendor interoperability is still unproven.",
        evidence=(
            "app/fhir_adapter.py",
            ...
            "docs/HOSPITAL_SELF_INSTALL_PLATFORM.md",
        ...
        blockers=(
            "one real approved KIS/LIS/vendor read-only sandbox",
            "production terminology service/validation for relevant code systems and value sets",
            "real cross-source patient identity/MPI mapping where a shared enterprise identifier is unavailable",
            "resource-version reconciliation/incremental synchronization against actual sources",
            "end-to-end freshness semantics against actual sources",
            "implement/test non-FHIR adapter families only where real integration demand requires them",
        ),
    ...
    Gate(
        ...
        status=GateStatus.PARTIAL,
        claim="OIDC verification, fail-closed authorization/context, secure-read orchestration, kill switches, audit protections, provider-side PHI architecture, deny-default deployment scaffolds, CodeQL, dependency audit/SBOM and security-governance foundations exist; production provider assurance remains incomplete.",
        evidence=(
            "app/auth_oidc.py",
    ...
    Gate(
        ...
        status=GateStatus.PARTIAL,
        claim="Fail-visible source semantics, bounded paging, kill switches, live-data startup lock, synthetic hospital upgrade preflight, safety failure-injection CI and a non-root container smoke test exist; target-environment resilience/recovery evidence remains missing.",
        evidence=(
            "app/source_state.py",
            "app/fhir_adapter.py",
            "app/connectors/fhir_connector.py",
        ...
        blockers=(
            ...
            "canary/rollback exercise in target deployment",
        ),
    ...
    Gate(
        ...
        status=GateStatus.PARTIAL,
        claim="Authenticated patient/treatment-context launch foundations, deterministic identity, an SJK synthetic workflow and a hospital-local context API exist; a real KIS/portal launcher and no-duplicate-work deployment are not yet proven.",
        evidence=(
    ...
    Gate(
        ...
        status=GateStatus.PARTIAL,
        claim="CareOS now has a proposal-grade assurance package plus an executable non-live hospital self-install/preflight/data-plane scaffold for Docker and Kubernetes. Hospital-specific completion, production controls and accountable approvals remain missing.",
        evidence=(
            "app/hospital_install.py",
            "app/hospital_runtime.py",
            "app/hospital_api.py",
        ...
        blockers=(
            ...
            "real hospital identity/context/audit/secret-management integration",
            "named support/incident/key/audit/rollback owners in the target provider",
    ...
    Gate(
        ...
        status=GateStatus.PARTIAL,
        claim="Stable connector/fact/failure contracts plus a machine-readable hospital manifest, adapter maturity catalog, deterministic preflight, upgrade guard and local data-plane packaging define the anti-fork scaling model. Repeatability is not proven until independent real hospital/vendor deployments succeed without a core fork.",
        evidence=(
            "app/connectors/base.py",
            "app/connectors/fhir_connector.py",
            "app/hospital_install.py",
        ...
        blockers=(
            "hospital A real approved read-only/deidentified integration",
            "hospital B with a different vendor/version",
            "measured custom engineering hours and adapter/test reuse per site",
            "real vendor compatibility records and upgrade regressions",
            "evidence that vendor differences remain adapter/configuration concerns rather than core forks",
        ),
    ...
    Gate(
        ...
        status=GateStatus.BLOCKED,
        claim="A proposal-ready federated German/EU reference architecture defines open contracts, a national integration map, migration/scaling principles and a procurement model; actual national integrations and multi-site operating evidence are not implemented.",
        evidence=(
...
def gate_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "reference_architecture_readiness": {
            ...
            "meaning": "architecturally reviewable for serious discussion; not production clearance or self-certification",
        },
