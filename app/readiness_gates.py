...
GATES: tuple[Gate, ...] = (
    Gate(
        ...
        evidence=("docs/ARCHITECTURE_V1.md", "docs/SAFETY_CASE.md"),
    ...
    Gate(
        ...
        claim="Canonical source-grounded fact model exists, but extraction reliability remains below clinical release bar.",
        evidence=("app/clinical_truth.py", "tests/test_clinical_truth.py", "docs/BENCHMARK.md"),
        blockers=("replace brittle extractor", "100% provenance on surfaced facts", "critical silent contradiction misses at release bar", "new frozen holdout"),
    ...
    Gate(
        ...
        status=GateStatus.BLOCKED,
        claim="Real generic FHIR transport is proven; German profile conformance and vendor sandbox are not.",
        evidence=("app/fhir_adapter.py", "docs/FHIR_INTEGRATION.md", "integration/docker-compose.fhir.yml"),
        blockers=("ISiK profile validation in CI", "terminology validation", "one real KIS/LIS/vendor read-only sandbox", "paging/version/freshness semantics"),
    ...
    Gate(
        ...
        status=GateStatus.BLOCKED,
        claim="Threat model and configuration gate exist; production identity/security/privacy controls do not.",
        evidence=("docs/THREAT_MODEL.md", "docs/PRODUCTION_READINESS.md", "app/security_readiness.py", "app/audit.py"),
        blockers=("working OIDC/SSO", "role/org/treatment-context authorisation", "immutable audit", "KMS/secrets", "DPIA/DSFA + DPA/AVV", "C5/customer-controls evidence where applicable", "independent penetration test"),
    ...
    Gate(
        ...
        status=GateStatus.BLOCKED,
        claim="Some source failures fail visibly, but system-wide resilience and recovery are not demonstrated.",
        evidence=("docs/THREAT_MODEL.md",),
        blockers=("failure-injection suite", "stale/degraded states across sources", "backup/restore evidence", "RPO/RTO", "rollback/kill switch", "operational monitoring/SLOs"),
    ...
    Gate(
        ...
        status=GateStatus.EXTERNAL_REVIEW,
        claim="Safety boundary is explicit; formal regulatory position and lifecycle/QMS are missing.",
        evidence=("docs/SAFETY_CASE.md",),
    ...
    Gate(
        ...
        claim="Architecture, threat model and readiness documents exist; procurement/assurance package is incomplete.",
        evidence=("docs/ARCHITECTURE_V1.md", "docs/THREAT_MODEL.md", "docs/PRODUCTION_READINESS.md"),
