...
GATES: tuple[Gate, ...] = (
    ...
    Gate(
        ...
        claim="Source-native FHIR passes through the mandatory truth contract; verified strong identifiers govern automatic patient attachment; untrusted document/model candidates require exact source evidence before promotion; governed unit conversion preserves original values. The legacy document benchmark remains below the clinical release bar.",
        evidence=("app/clinical_truth.py", "app/patient_identity.py", "app/fhir_adapter.py", "app/document_pipeline.py", "app/extractors/base.py", "app/extractors/model_schema.py", "app/contradictions.py", "app/unit_normalization.py", "tests/test_clinical_truth.py", "tests/test_patient_identity.py", "tests/test_fhir_truth.py", "tests/test_document_pipeline.py", "tests/test_verified_extraction.py", "tests/test_model_extraction_schema.py", "tests/test_contradictions.py", "tests/test_unit_normalization.py", "docs/BENCHMARK.md"),
    ...
    Gate(
        ...
        claim="Real FHIR transport, bounded same-origin Bundle pagination and pinned gematik ISiK5 profile-validation CI are proven. CareOS now explicitly separates profile validation from terminology evidence; a real terminology service, synchronization semantics and hospital/vendor sandbox remain missing.",
        evidence=("app/fhir_adapter.py", "app/connectors/base.py", "app/connectors/fhir_connector.py", "app/terminology_policy.py", "tests/test_fhir_paging.py", "tests/test_connector_contract.py", "tests/test_terminology_policy.py", "docs/FHIR_INTEGRATION.md", "docs/CONNECTOR_SDK.md", "docs/TERMINOLOGY_VALIDATION.md", "integration/docker-compose.fhir.yml", "integration/isik5/Patient-careos-synthetic.json", ".github/workflows/isik5-validation.yml"),
        blockers=("real terminology validation for code systems/value sets excluded by the pinned ISiK validator", "one real KIS/LIS/vendor read-only sandbox", "resource version reconciliation/incremental synchronization", "end-to-end freshness semantics against a real source"),
    ...
    Gate(
        ...
        status=GateStatus.PARTIAL,
        claim="A short-lived authenticated organisation-, patient- and treatment-context-bound launch contract exists, with deterministic patient-identity rules; a real KIS/portal launcher and no-duplicate-work deployment are not yet proven.",
        evidence=("app/context_launch.py", "tests/test_context_launch.py", "app/patient_identity.py", "tests/test_patient_identity.py", "app/auth_oidc.py", "app/access_policy.py", "share/"),
