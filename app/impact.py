...
def score_pilot_task(task_id: str, baseline_minutes: float, actual_seconds: float, *, clicks: int = 0,
    ...
    saved = max(0.0, baseline_minutes - actual_minutes)
    reduction = max(0.0, min(100.0, (saved / baseline_minutes) * 100.0))
    return {
        "task_id": task_id,
...
def aggregate_results(results: list[dict]) -> dict:
    ...
    return {
        ...
        "median_saved_minutes": round(median(float(r.get("saved_minutes", 0)) for r in results), 2),
        "total_saved_minutes": round(sum(float(r.get("saved_minutes", 0)) for r in results), 2),
