...
class ConservativeGermanExtractor:
    ...
    This is intentionally *not* a general clinical NLP system. It only extracts a
    small set of explicit forms from the development corpus. Unsupported high-risk
    documents become review-required instead of silently producing an empty result.
    ...
    @staticmethod
    def _candidate(document: DocumentInput, *, match: re.Match[str], fact_type: str,
                   value_original, value_normalized=None, unit_original: str | None = None,
                   unit_normalized: str | None = None, effective_time: datetime | None = None,
                   status: FactStatus = FactStatus.CONFIRMED, review_reason: str | None = None,
                   confidence: float = 1.0) -> ExtractedCandidate:
    ...
    def _allergy(self, document: DocumentInput) -> list[ExtractedCandidate]:
        ...
        return [self._candidate(document, match=match, fact_type="allergy", value_original=value)]
    ...
    def _medication(self, document: DocumentInput) -> list[ExtractedCandidate]:
        ...
        if new_order:
            out.append(self._candidate(document, match=new_order, fact_type="new_medication_order", value_original=new_order.group(1).strip()))
        return out
    ...
    def _diagnosis(self, document: DocumentInput) -> list[ExtractedCandidate]:
        ...
        return [self._candidate(document, match=match, fact_type="relevant_diagnoses", value_original=diagnoses)]
    ...
    def _lab(self, document: DocumentInput) -> list[ExtractedCandidate]:
        ...
        return [
            self._candidate(
                ...
                value_original={"creatinine": creatinine, "egfr": egfr},
                unit_original="mg/dl + ml/min/1.73m2",
            )
        ]
